...
// Load environment
dotenv_1.default.config({ path: '.env.local.runner' });
// CLI configuration
commander_1.program
...
const config = {
    ...
    sessionRoot: process.env.SESSION_ROOT || path.join((0, os_1.homedir)(), '.linkedin-runner', 'sessions'),
...
const logger = winston_1.default.createLogger({
    ...
    transports: [
        ...
        new winston_1.default.transports.File({
            filename: path.join((0, os_1.homedir)(), '.linkedin-runner', 'logs', 'runner.log'),
...
// Load selector pack
async function loadSelectorPack(version) {
...
class LinkedInRunner {
    ...
    async initialize() {
        ...
        this.browser = await playwright_1.chromium.launchPersistentContext(userDataDir, {
            headless: false,
        ...
        await this.context.addInitScript(() => {
            ...
            Object.defineProperty(globalThis.navigator, 'plugins', { get: () => [1, 2, 3, 4, 5] });
        });
    }
    async checkHealth() {
        try {
    ...
    async backupSession() {
        try {
            ...
            const { error } = await supabase.storage
                ...
                .upload(`${this.userId}/${this.accountId}/backup-${Date.now()}.json`, JSON.stringify(sessionData), { contentType: 'application/json' });
            if (!error) {
    ...
    async processTask(task) {
        ...
        try {
            ...
            const messageButton = await this.page.locator(`${this.selectors.selectors.messaging.messageButton}, ${this.selectors.selectors.messaging.messageButtonAlt}`).first();
            if (await messageButton.isVisible()) {
                ...
                if (await messageBox.isVisible()) {
                    ...
                    // Screenshot
                    const screenshotPath = path.join((0, os_1.homedir)(), '.linkedin-runner', 'screenshots', `${task.id}.png`);
                    await fs.mkdir(path.dirname(screenshotPath), { recursive: true });
                    await this.page.screenshot({ path: screenshotPath });
                    if (!options.dryRun) {
                        // Send message
                        const sendButton = await this.page.locator(`${this.selectors.selectors.messaging.sendButton}, ${this.selectors.selectors.messaging.sendButtonAlt}`).first();
